...
def main(args):
    ...
    subjects = sorted([
        f.replace("_dev.csv", "")
        for f in os.listdir(os.path.join(args.data_dir, "dev"))
    ...
    subject_accuracies = result_data["subjects"]

    # 循环评估
    for subject in subjects:
        if subject in subject_accuracies:
            print(f"⚠️ 已存在 {subject} 结果，跳过。")
            continue
        ...
        pd.DataFrame(cors, columns=["gold", "pred"]).to_csv(
            os.path.join(args.save_dir, f"{subject}.csv"), index=False
        )
...
if __name__ == "__main__":
    ...
    parser.add_argument("--subjects", type=str, default="all", help="单学科名（逗号分隔）或'all'")
